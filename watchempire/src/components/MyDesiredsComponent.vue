        ...
        <div class="relative overflow-x-auto mt-12">
          <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-300">
              <tr>
                <th scope="col" class="px-6 py-3 border text-center text-base">
                  Producto
                </th>
                <th scope="col" class="px-6 py-3 border text-center text-base">
                  Imagen
            ...
            <tbody v-for="product in products" :key="product.id">
              <tr class="bg-white border-b">
                <td
                  scope="row"
                  class="px-6 py-4 font-medium text-xl text-gray-900 whitespace-nowrap border text-center"
                >
                  {{ product.name }}
                </td>
                <td class="px-6 py-4 border text-center">
                  <img
                    class="h-32 m-auto rounded-full object-cover transition duration-300 transform hover:scale-110"
                    alt="Imagen de producto"
                    :src="require('@/assets/img/' + product.image)"
                  />
                </td>
                <td class="px-6 py-4 border text-center text-2xl">
                  {{ product.price }} $
                </td>
                <td class="px-6 py-4 border text-center text-xl">
                  <button
